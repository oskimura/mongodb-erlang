...
add_user (Permission, Username, Password) ->
	User = case find_one (binary_to_atom(<<"system.users">>, latin1), {user, Username}) of {} -> {user, Username}; {Doc} -> Doc end,
	Rec = {readOnly, case Permission of read_only -> true; read_write -> false end, pwd, pw_hash (Username, Password)},
	save (binary_to_atom(<<"ssystem.users">>, latin1), bson:merge (Rec, User)).
